...
def load_data(data_dir):
    ...
    for directory_name in os.listdir(images_directory):
        current = os.path.join(images_directory,directory_name)
        if os.path.isdir(current):
            for filename in os.listdir(current):
                ...
                alteration = (IMG_WIDTH,IMG_HEIGHT)
...
def get_model():
    ...
    model = tf.keras.models.Sequential([
        ...
        ## Note: Test more permutations of this basic convolution layer
        # 
        ...
        # Max-pooling layer for the second convolutional layer
        tf.keras.layers.MaxPooling2D(pool_size=(2, 2)),
        
        # Flatten units
        tf.keras.layers.Flatten(),

        # Add a hidden layer with dropout - 128 
        tf.keras.layers.Dense(600, activation="relu", name='layer1'),
